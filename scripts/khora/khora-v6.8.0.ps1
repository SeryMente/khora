...
# ================================================================
$SCRIPT_VERSION = "6.8.0"   # <- UNICA fuente de verdad de la version
...
$CDP_PORT    = 9333
$TAB_SNAPSHOT_PATH = Join-Path $REPO_DIR "session-state\chrome-tabs.json"
...
function Unprotect-KhoraToken {
    ...
    for ($i = 0; $i -lt $expectedTag.Length; $i++) {
        if ($actualTag[$i] -ne $expectedTag[$i]) {
            throw "El tag de integridad HMAC no coincide. Contrasena incorrecta o token corrupto."
        }
    }
    ...
    foreach ($v in $EnvManifest) {
        ...
        if ($vault.ContainsKey($name)) {
            ...
            if ($vercelToken -and $targets -contains "Vercel") {
                $webDir = Join-Path $REPO_DIR "khora-web"
                # Intentamos leer de Vercel (vercel env ls / env pull no dan un JSON simple,
                # usaremos env pull a temporal y parseamos, o mas facil asuminos fallback a Read-Host)
                # "intentar recuperar el valor desde Vercel: vercel env pull"
                $tmpEnv = Join-Path $env:TEMP "vercel-pull-$([guid]::NewGuid().ToString()).env"
                & cmd /c "cd /d `"$webDir`" && vercel env pull `"$tmpEnv`" --environment production --token $vercelToken -y >nul 2>&1"
                if (Test-Path $tmpEnv) {
            ...
            if (-not $foundInCloud -and $renderApiToken -and $renderSvcId -and $targets -contains "Render") {
                ...
                $url = "https://api.render.com/v1/services/$renderSvcId/env-vars"
                try {
