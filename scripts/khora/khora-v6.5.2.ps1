...
#  - Instalacion forzada de LastPass si falta, revirtiendo al cerrar (v6.5.2)
# ================================================================
param(
...
# ================================================================
$SCRIPT_VERSION = "6.5.2"   # <- UNICA fuente de verdad de la version
...
$script:LASTPASS_INSTALLED_BY_SCRIPT = $false
# ================================================================
...
# TODO-RECONCILIAR deduplicar Test-LastPassInstalled con khora-v6.6.0.ps1 (rama paralela)
function Test-LastPassInstalled {
    $lpId = "hdokiejnpimakedhajhdlcegeplioahd"
    # TODO-RECONCILIAR migrar $env:LOCALAPPDATA a $script:REAL_USER_PROFILE cuando se integre v6.5.1
    $extPath = Join-Path $env:LOCALAPPDATA "Google\Chrome\User Data\*\Extensions\$lpId"
    return (Test-Path $extPath)
}

function Install-LastPassExtensionIfMissing {
    if (Test-LastPassInstalled) {
        Ok "LastPass ya esta instalado. No se modifica."
        return
    }
    ...
    New-ItemProperty -Path $keyPath -Name "1" -PropertyType String -Force -Value "hdokiejnpimakedhajhdlcegeplioahd;https://clients2.google.com/service/update2/crx" | Out-Null
    $script:LASTPASS_INSTALLED_BY_SCRIPT = $true
    Ok "Politica de instalacion forzada escrita (alcance: usuario actual, HKCU). Se instalara automaticamente cuando Chrome abra esta sesion."
}

function Remove-LastPassForcedInstall {
    if (-not $script:LASTPASS_INSTALLED_BY_SCRIPT) {
        return
    }

    $keyPath = "HKCU:\Software\Policies\Google\Chrome\ExtensionInstallForcelist"
    Remove-ItemProperty -Path $keyPath -Name "1" -ErrorAction SilentlyContinue

    try {
        if ((Get-Item $keyPath -ErrorAction Stop).Property.Count -eq 0) {
            Remove-Item -Path $keyPath -Force -ErrorAction SilentlyContinue
        }
    } catch {
        # Clave probablemente ya no existe
    }

    Ok "Politica de LastPass revertida (sin rastro en la maquina)."
}
...
"@
    ...
    Install-LastPassExtensionIfMissing
