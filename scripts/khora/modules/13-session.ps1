...
function Start-Sesion {
    Write-InitHeader
    try{
        Invoke-KhoraStage -Id 'EP-IN-070' -Label 'Autorizar GitHub y reconstruir continuidad privada' -Action {if(-not(Test-KhoraEncryptedWorkspace)){throw'Volumen no cifrado.'};Import-KhoraBootstrapSecrets;if(-not(Test-KhoraGitHubToken)){throw'Personal Access Token de GitHub inválido.'};Start-KhoraPrefetch;if(-not(Ensure-Git)){throw'Git no disponible.'};if(-not(Confirm-GhCliAuth)){throw'GitHub CLI no autorizado.'};Initialize-KhoraRepository;Init-Wip}|Out-Null
        . (Join-Path $REPO_DIR 'scripts\khora\env-vault.ps1')
        Invoke-KhoraStage -Id 'EP-IN-080' -Label 'Abrir la bóveda y autorizar Vercel' -Action {$loaded=Import-KhoraEnvVault -Names @('VERCEL_TOKEN');if($loaded-notcontains'VERCEL_TOKEN'){throw'VERCEL_TOKEN ausente.'};Connect-KhoraVercel}|Out-Null
