...
Assert-Khora($gate-match"EP_VERSION\s*=\s*'1\.0\.1'")'Version EP incorrecta.'
Assert-Khora($gate-match"SCRIPT_VERSION\s*=\s*'7\.5\.1'")'Version host incorrecta.'
...
if($errors.Count){$errors|ForEach-Object{Write-Error $_};exit 1};Write-Host 'Validacion estatica EP v1.0.1: OK' -ForegroundColor Green
