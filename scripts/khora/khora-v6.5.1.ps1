...
#  - Endurecimiento de deteccion de usuario real con triangulacion (v6.5.1)
# ================================================================
param(
...
# ================================================================
$SCRIPT_VERSION = "6.5.1"   # <- UNICA fuente de verdad de la version
...
# ================================================================
$script:REAL_USER            = $null    # nombre del usuario real detectado
$script:REAL_USER_CONFIDENCE = $null    # ALTA, MEDIA, BAJA-DISCREPANCIA, DEGRADADA
$script:REAL_USER_PROFILE    = $null    # ruta real del perfil
$script:REAL_USER_DETECT_LOG = @()      # bitacora de deteccion
$script:REAL_USER_OVERRIDE   = $false   # $true si se redireccionaron env vars

function Get-SessionOwnerCandidate {
    $c = [pscustomobject]@{ SessionUser = $null; ExplorerUser = $null }
    try { $sid = (Get-Process -Id $PID).SessionId } catch { return $c }

    # Fuente A: quser / query session
    try {
        $q = $null
        try { $q = & query session 2>$null } catch { $q = $null }
        if (-not $q) { try { $q = & qwinsta 2>$null } catch { $q = $null } }
        if ($q) {
            foreach ($ln in @($q)) {
                $t = ("$ln" -replace '^[>\s]+','')
                # Buscamos la fila que tenga el mismo SessionId
                if ($t -match "^\S+\s+(\S+)\s+$sid\s+") {
                    $c.SessionUser = $Matches[1].Trim()
                    break
                } elseif ($t -match "^(\S+)\s+$sid\s+") { # a veces la primera columna esta vacia (nombre de sesion) y empieza por username
                    $c.SessionUser = $Matches[1].Trim()
                    break
                }
            }
    ...
    # Fuente B: explorer.exe en la misma sesion
    try {
        $exps = @(Get-CimInstance Win32_Process -Filter "Name='explorer.exe'" -ErrorAction Stop)
        foreach ($p in $exps) {
            try {
                $esid = (Get-Process -Id $p.ProcessId -ErrorAction Stop).SessionId
                if ($esid -eq $sid) {
                    $o = Invoke-CimMethod -InputObject $p -MethodName GetOwner -ErrorAction Stop
                    if ($o -and $o.User) {
                        $c.ExplorerUser = $o.User.Trim()
                        break
                    }
                }
            } catch {}
...
function Resolve-RealUserIdentity {
    $c = Get-SessionOwnerCandidate
    $script:REAL_USER_DETECT_LOG += "SessionUser: $($c.SessionUser) | ExplorerUser: $($c.ExplorerUser)"

    if ($c.SessionUser -and $c.ExplorerUser -and ($c.SessionUser -ieq $c.ExplorerUser)) {
        $script:REAL_USER = $c.SessionUser
        $script:REAL_USER_CONFIDENCE = "ALTA"
    } elseif ($c.SessionUser -and -not $c.ExplorerUser) {
        $script:REAL_USER = $c.SessionUser
        $script:REAL_USER_CONFIDENCE = "MEDIA"
        $script:REAL_USER_DETECT_LOG += "[WARN] Deteccion de usuario: ExplorerUser vacio. Confianza MEDIA."
    } elseif (-not $c.SessionUser -and $c.ExplorerUser) {
        $script:REAL_USER = $c.ExplorerUser
        $script:REAL_USER_CONFIDENCE = "MEDIA"
        $script:REAL_USER_DETECT_LOG += "[WARN] Deteccion de usuario: SessionUser vacio. Confianza MEDIA."
    } elseif ($c.SessionUser -and $c.ExplorerUser -and ($c.SessionUser -ine $c.ExplorerUser)) {
        $script:REAL_USER = $c.ExplorerUser
        $script:REAL_USER_CONFIDENCE = "BAJA-DISCREPANCIA"
        $msg = "Discrepancia en deteccion de usuario real. SessionUser: $($c.SessionUser) vs ExplorerUser: $($c.ExplorerUser). Usando ExplorerUser."
        $script:REAL_USER_DETECT_LOG += "[FAIL] $msg"
        $script:REAL_USER_DETECT_LOG += "[CRIT] $msg"
    } else {
        $script:REAL_USER = $env:USERNAME
        $script:REAL_USER_CONFIDENCE = "DEGRADADA"
        $script:REAL_USER_DETECT_LOG += "[FAIL] Usando cuenta del proceso elevado como ultimo recurso - alto riesgo de rutas incorrectas."
    }

    # Resolver perfil real
    $resolvedPath = $null
    try {
        $users = Get-CimInstance Win32_UserProfile -ErrorAction Stop
        foreach ($u in $users) {
            try {
                $sidObj = New-Object System.Security.Principal.SecurityIdentifier($u.SID)
                $acc = $sidObj.Translate([System.Security.Principal.NTAccount])
                $accName = ($acc.Value -split '\\')[-1]
                if ($accName -ieq $script:REAL_USER) {
                    $resolvedPath = $u.LocalPath
                    break
                }
            } catch {}
        }
    ...
    if (-not $resolvedPath -or -not (Test-Path $resolvedPath)) {
        $fallbackPath = Join-Path $env:SystemDrive "Users\$($script:REAL_USER)"
        if (Test-Path $fallbackPath) { $resolvedPath = $fallbackPath }
    }

    if ($resolvedPath) {
        $script:REAL_USER_PROFILE = $resolvedPath
        $script:REAL_USER_DETECT_LOG += "Perfil detectado: $resolvedPath"

        if ($env:USERNAME -ine $script:REAL_USER) {
            $env:USERNAME     = $script:REAL_USER
            $env:USERPROFILE  = $resolvedPath
            $env:LOCALAPPDATA = Join-Path $resolvedPath "AppData\Local"
            $env:APPDATA      = Join-Path $resolvedPath "AppData\Roaming"
            $env:HOMEDRIVE    = $env:SystemDrive
            $env:HOMEPATH     = "\Users\$($script:REAL_USER)"
            $script:REAL_USER_OVERRIDE = $true
            $script:REAL_USER_DETECT_LOG += "REDIRECCION APLICADA: env vars redirigidas al usuario real."
        }
    } else {
        $script:REAL_USER_PROFILE = $env:USERPROFILE
        $script:REAL_USER_DETECT_LOG += "No se pudo resolver el perfil en disco; usando env:USERPROFILE actual."
    }
}

Resolve-RealUserIdentity
# ================================================================
...
function Start-Sesion {
    ...
    # v6.4.6/v6.5.1: bitacora completa de la deteccion multi-metodo al log
    foreach ($__dl in @($script:REAL_USER_DETECT_LOG)) { L "INFO" "DeteccionUsuarioReal $__dl" }

    Info "Cuenta del proceso elevado: $($env:USERNAME)"
    Info "Usuario real detectado: $($script:REAL_USER) [confianza: $($script:REAL_USER_CONFIDENCE)]"

    if ($script:REAL_USER_OVERRIDE) {
        Ok   "Usuario de trabajo : $($script:REAL_USER) (perfil: $env:USERPROFILE)"
        Info "Elevado como admin : $($env:USERNAME) (su perfil NO se usara, env vars redirigidas)"
        Ok   "Workdir, repo, venv, .env -> quedan en el perfil de $($script:REAL_USER)."
    } else {
        if ($script:REAL_USER_CONFIDENCE -eq "ALTA" -or $script:REAL_USER_CONFIDENCE -eq "MEDIA") {
            Ok   "Usuario real ($($script:REAL_USER)) == usuario del proceso: mismo usuario, sin redireccion necesaria."
        }
...
"@
    ...
    $profileTxt = if ($script:REAL_USER_OVERRIDE) { "$($script:REAL_USER) [elevado]" } else { $env:USERNAME }
...
function Run-Main {
    ...
    if ($script:REAL_USER_OVERRIDE) {
        ...
        Write-Host "   Usuario de trabajo : $($script:REAL_USER)" -ForegroundColor Green
        Write-Host "   Cuenta admin original : (redirigida)" -ForegroundColor Cyan
        Write-Host "   Perfil de trabajo  : $env:USERPROFILE" -ForegroundColor Green
        Write-Host "   Todas las instalaciones y datos van al perfil de $($script:REAL_USER)." -ForegroundColor Green
        Write-Host "  ============================================================" -ForegroundColor Yellow
        Write-Host ""
        L "INFO" "Elevacion mixta: usuario=$($script:REAL_USER) perfil=$env:USERPROFILE"
    }
    Open-LogWindow
    ...
    while ($true) {
        ...
        if ($script:SES_ACTIVE -and (Get-Date) -ge $nextWip) { Do-AutoWip; $nextWip = (Get-Date).AddMinutes($CFG.autoWipMinutes) }
