...
#  - Persistencia cifrada del token (PAT) entre sesiones (v6.5.3)
# ================================================================
param(
...
# ================================================================
$SCRIPT_VERSION = "6.5.3"   # <- UNICA fuente de verdad de la version
...
function Ensure-GitignoreHygiene {
    ...
    $needsAdd = $true
    $needsTokenExc = $true
    $tokenPat = "!session-state/gh-token.enc.json"
    $tokenPatRe = "^!session-state/gh-token\.enc\.json$"

    if (Test-Path $gi) {
        $lines = Get-Content $gi -ErrorAction SilentlyContinue
        if ($lines -match $patRe) { $needsAdd = $false }
        if ($lines -match $tokenPatRe) { $needsTokenExc = $false }
    }
    if ($needsAdd) {
    ...
    if ($needsTokenExc) {
        Add-Content $gi "`n$tokenPat" -Encoding UTF8 -ErrorAction SilentlyContinue
        Ok "Excepcion $tokenPat agregada a .gitignore"
    }
...
function Save-TokenSnapshot {
    ...
    # Si el repo ya existe (ej: estamos en medio de sesion y roto), guardamos y pusheamos
    if (Test-Path "$REPO_DIR/.git") {
        $stateDir = Join-Path $REPO_DIR "session-state"
        if (-not (Test-Path $stateDir)) { New-Item -ItemType Directory -Path $stateDir -Force | Out-Null }
        $outFile = Join-Path $stateDir "gh-token.enc.json"
        $snapshot | ConvertTo-Json -Depth 5 | Set-Content $outFile -Encoding UTF8 -Force

        git -C $REPO_DIR add $outFile 2>&1 | Out-Null
        git -C $REPO_DIR commit -m "chore(session): actualizar token cifrado" 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            $branch = (git -C $REPO_DIR branch --show-current).Trim()
            Push-Verified -Branch $branch -Retries 2 | Out-Null
            Ok "Snapshot de token guardado y sincronizado."
        } else {
            Warn "Snapshot guardado localmente, pero no se pudo hacer commit."
        }
    } else {
        # Si no esta clonado aun, lo guardamos en memoria para escribirlo y pushearlo justo despues del clone
        $script:PendingTokenSnapshot = $snapshot
        Info "Snapshot cifrado generado, se guardara en disco y repositorio despues del clone."
    }
}

function Test-TokenSnapshotValid {
    $outFile = Join-Path $REPO_DIR "session-state/gh-token.enc.json"
    $snapshotJson = $null
...
function Start-Sesion {
    ...
    # Escribir snapshot pendiente de la autenticacion si lo hay
    if ($script:PendingTokenSnapshot) {
        $stateDir = Join-Path $REPO_DIR "session-state"
        if (-not (Test-Path $stateDir)) { New-Item -ItemType Directory -Path $stateDir -Force | Out-Null }
        $outFile = Join-Path $stateDir "gh-token.enc.json"
        $script:PendingTokenSnapshot | ConvertTo-Json -Depth 5 | Set-Content $outFile -Encoding UTF8 -Force

        git -C $REPO_DIR add $outFile 2>&1 | Out-Null
        git -C $REPO_DIR commit -m "chore(session): actualizar token cifrado" 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            $currBranch = (git -C $REPO_DIR branch --show-current).Trim()
            Push-Verified -Branch $currBranch -Retries 2 | Out-Null
            Ok "Snapshot de token guardado y sincronizado tras clone."
        }
        $script:PendingTokenSnapshot = $null
    }
