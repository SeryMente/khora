...
#  - Limpieza resiliente, P/Invoke Ctrl+C, push via gh auth, menu reducido (v6.5.4)
# ================================================================
param(
...
# ================================================================
$SCRIPT_VERSION = "6.5.4"   # <- UNICA fuente de verdad de la version
...
function Invoke-Cleanup {
    ...
    try {
        ...
        if (Test-Path $repoLog) {
            ...
            # Intentar pushear si hay token O si gh CLI está configurado
            ...
            } else { Info "Sin token ni gh auth (limpieza externa): push omitido; la compuerta de borrado revisara si quedo trabajo sin respaldo." }
        }
...
function Run-Main {
    ...
    # Cierre garantizado si se cierra con la X o error (CTRL_CLOSE_EVENT nativo + PowerShell.Exiting)
    try {
        $sigCtrl = @"
using System;
using System.Runtime.InteropServices;
