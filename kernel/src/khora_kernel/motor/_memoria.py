...
class Neo4jMemoriaOrganizada:
    ...
    def escribir_ingesta(self, triples: List[Triple], provenance: Provenance, io_id=None, terna_volcado: Optional[dict] = None) -> int:
        self._asegurar_conexion()
        if not provenance:
    ...
    def fusionar_ingesta(self, triples: List[Triple], provenance: Provenance, io_id=None, terna_volcado: Optional[dict] = None) -> int:
        self._asegurar_conexion()
        if not provenance:
            raise Exception("No se puede escribir sin provenance.")
