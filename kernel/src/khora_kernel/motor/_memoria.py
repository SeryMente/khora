...
from neo4j import Driver, GraphDatabase # type: ignore  # pyright: ignore[reportMissingImports, reportUnknownVariableType]
...
class Neo4jMemoriaOrganizada:
    ...
    def buscar_entidades_candidatas(self, label_norm: str) -> typing.List[typing.Dict[str, typing.Any]]:
        ...
        try:
            assert self._driver is not None
            with self._driver.session() as session:
                result = session.run(query, label_norm=label_norm)  # type: ignore
                candidatos = []
                for record in result:
                    candidatos.append({
    ...
    def merge_entidad(self, canonical_key: str, label_original: str, provenance_raw: str, embedding: typing.List[float], matiz_de: typing.Optional[str] = None, needs_review: bool = False) -> None:
        ...
        try:
            assert self._driver is not None
            with self._driver.session() as session:
                session.run(query, canonical_key=canonical_key, label_original=label_original, embedding=embedding, provenance_raw=provenance_raw, needs_review=needs_review)  # type: ignore
                if matiz_de:
                    session.run(rel_query, canonical_key=canonical_key, matiz_de=matiz_de)  # type: ignore
        except Exception as e:
