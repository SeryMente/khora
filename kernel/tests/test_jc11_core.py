...
def test_multihop(test_db_path, mock_memoria, monkeypatch):
    ...
    class MockProvider:
        def generar(self, solicitud):
            ...
            return RespuestaLLM(texto=ans, modelo="mock", provenance=Provenance(origen="mock", driver=None, timestamp=datetime.now(timezone.utc).isoformat()))
...
def test_ht_persistido(test_db_path, mock_memoria, monkeypatch):
    class MockProvider:
        def generar(self, solicitud):
            from khora_kernel.api import RespuestaLLM, Provenance
            from datetime import datetime, timezone
            return RespuestaLLM(texto="Hola [comm1].", modelo="mock", provenance=Provenance(origen="mock", driver=None, timestamp=datetime.now(timezone.utc).isoformat()))
...
def test_no_escritura_pkg(test_db_path, mock_memoria, monkeypatch):
    class MockProvider:
        def generar(self, solicitud):
            from khora_kernel.api import RespuestaLLM, Provenance
            from datetime import datetime, timezone
            return RespuestaLLM(texto="Algo.", modelo="mock", provenance=Provenance(origen="mock", driver=None, timestamp=datetime.now(timezone.utc).isoformat()))
...
def test_esquemas(test_db_path, mock_memoria, monkeypatch):
    class MockProvider:
        def generar(self, solicitud):
            from khora_kernel.api import RespuestaLLM, Provenance
            from datetime import datetime, timezone
            return RespuestaLLM(texto="Esquemas [node1].", modelo="mock", provenance=Provenance(origen="mock", driver=None, timestamp=datetime.now(timezone.utc).isoformat()))
