...
# Fix the duplicate node creation issue due to random hash addition in NEW fallback when not colliding properly or always triggering.
# Also fix provenance accumulation logic if there are duplicates contexts.
# The bug is likely around line 133: `if any(c["canonical_key"] == canonical for c in candidatos_memoria):`
# Also need to make sure we don't accidentally iterate nodes we already have in mapping for subsequent resolutions.
