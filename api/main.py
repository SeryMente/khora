...
neo4j_config.update({"uri": uri, "user": user, "password": password})
try:
...
try:
memoria = Neo4jMemoriaOrganizada(neo4j_config["uri"], neo4j_config["user"], neo4j_config["password"])
...
Salida buena: **"1 file changed"** + push a `main`. Pégamela y arranca el reloj de Render (~5 min) para la prueba de fuego.
​
nO PUEDES MEJOR EN UN SOLO COMANDO PARA QUE NO TENGA QUE ABRIR EL ARCHIVO?
18:54
Pensamiento
Sí — mejor aún: este bloque aplica el parche él solo (lee api/main.py, hace las 4 ediciones quirúrgicas en memoria, verifica que quedaron bien, y solo entonces publica). Un solo pegado, cero pausas, cero archivos abiertos:
& {
    Set-Location "C:\Users\015RENTA01\AppData\Local\khora-session\repo"
    git fetch origin
    git checkout main
    git pull origin main 2>&1 | Select-Object -Last 1
    $LF = "`n"
    $p = (Resolve-Path "api\main.py").Path
    $t = [IO.File]::ReadAllText($p)
    $t = $t.Replace('neo4j_driver = None', 'neo4j_driver = None' + $LF + 'neo4j_config = {}')
    $t = $t.Replace('        neo4j_driver = GraphDatabase.driver(uri, auth=(user, password))', '        neo4j_config.update({"uri": uri, "user": user, "password": password})' + $LF + '        neo4j_driver = GraphDatabase.driver(uri, auth=(user, password))')
    $t = $t.Replace('from khora_kernel.drivers.memoria import MemoriaNeo4j', 'from khora_kernel.motor import Neo4jMemoriaOrganizada')
    $t = $t.Replace('memoria = MemoriaNeo4j(neo4j_driver)', 'memoria = Neo4jMemoriaOrganizada(neo4j_config["uri"], neo4j_config["user"], neo4j_config["password"])')
    $t = $t.Replace('        logging.error(f"Ingest error: {traceback.format_exc()}")', '        logging.error(f"Ingest error: {traceback.format_exc()}")' + $LF + '        if isinstance(e, ImportError):' + $LF + '            raise HTTPException(status_code=503, detail={"error": "motor no disponible", "causa": str(e)})')
    if (-not ($t.Contains('Neo4jMemoriaOrganizada') -and -not $t.Contains('drivers.memoria'))) { Write-Host "PARCHE NO APLICADO: no se publico nada. Pegame este mensaje."; return }
    [IO.File]::WriteAllText($p, $t)
    git add -A
    git commit -m "fix(api): puente al motor real Neo4jMemoriaOrganizada + 503 honesto en ingesta"
