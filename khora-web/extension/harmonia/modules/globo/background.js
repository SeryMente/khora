...
let _lastSpikeAt = 0;
let _lastBoostCutAt = 0;
let _audWin = null;
function _audNum(v){ return (typeof v === "number" && isFinite(v)) ? v : null; }
function _audReset(now){ _audWin = { startedAt: now || Date.now(), n:0, sumConceal:0, maxConceal:0, sumLoss:0, maxLoss:0, dLost:0, sumJank:0, maxJank:0, jankN:0, longtasks:0, longtaskMs:0, accel:0, decel:0, sumJbMs:0, maxJbMs:0, jbN:0, spikes:0, boostSum:0, boostN:0, boostMax:0, boostCuts:0 }; _bgTouchedInWin = false; }
...
// Correlacion boost<->calidad: resume el efecto del volume booster en la ventana forense.
function _boostCorr(w){ try{ if(!w || w.boostMax <= 1.0) return "boost:off"; const conceal = w.n ? (w.sumConceal/w.n) : 0; const loss = w.n ? (w.sumLoss/w.n) : 0; return "boost:"+(+w.boostMax).toFixed(2)+"x conceal:"+(Math.round(conceal*1000)/10)+"% perdida:"+(Math.round(loss*1000)/10)+"% cortes:"+w.boostCuts; }catch(e){ return null; } }
async function flushAudioWindow(reason){
  try{
    ...
    const data = {
      ...
      boost_gain_medio: w.boostN ? +(w.boostSum/w.boostN).toFixed(2) : null, boost_gain_max: +(w.boostMax).toFixed(2), boost_cortes: w.boostCuts, boost_activo: w.boostMax > 1.0, correlacion_boost: _boostCorr(w),
      picos: w.spikes, condicion: await _condLabel(), actividad_extension: _bgTouchedInWin, motivo: reason || "ventana"
    };
    const bad = (w.maxConceal >= AUD_SPIKE_CONCEAL) || (w.maxJank >= AUD_SPIKE_JANK) || (w.dLost >= AUD_SPIKE_LOST) || (w.spikes > 0) || (w.boostCuts > 0);
...
async function openAhkUi(){
  try{
    const url = chrome.runtime.getURL("modules/globo/options.html#ahk");
    const baseUrl = chrome.runtime.getURL("modules/globo/options.html");
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse)=>{
  (async ()=>{
    ...
    const _allowWithoutAhk = (msg.type === "ahkCheck" || msg.type === "logClient" || msg.type === "overlayCmd" || msg.type === "safeCfg" || msg.type === "audioStat" || msg.type === "boosterStat");
    ...
    if(msg.type === "log"){ await logEvent(msg.level || "info", msg.msg, msg.data, msg.mod || "content"); }
      ...
      const bg = _audNum(m.boostGain);
      if(bg!=null){ w.boostSum += bg; w.boostN++; if(bg>w.boostMax) w.boostMax = bg; }
    ...
    else if(msg.type === "boosterStat"){
      // Auto-telemetria del volume booster (mundo ISOLATED): gain y, sobre todo, CORTES de
      // audio con boost activo (silencio detectado por el AnalyserNode). Alimenta la
      // correlacion gain<->calidad y dispara un evento inmediato ante cada corte.
      const now = Date.now();
      const st = (msg && typeof msg === "object") ? msg : {};
      const g = _audNum(st.gain);
      await chrome.storage.local.set({ boosterStat: { at: now, st: st } });
      if(!_audWin) _audReset(now);
      const w = _audWin;
      if(g!=null && g>w.boostMax) w.boostMax = g;
      if(st.kind === "silence"){
        w.boostCuts++;
        if(now - _lastBoostCutAt >= AUD_SPIKE_COOLDOWN_MS){
          _lastBoostCutAt = now;
          const cond = await _condLabel();
          await logEvent("warn","CORTE de audio con boost activo: gain "+(g!=null?(g.toFixed(2)+"x"):"\u00b7")+", silencio ~"+(_audNum(st.silentMs)!=null?Math.round(st.silentMs)+"ms":"\u00b7")+" \u00b7 ["+cond+"] \u00b7 ext:"+(_bgTouchedInWin?"si":"no"), st, "livecall");
        }
      }
      if(sendResponse) sendResponse({ ok:true }); return;
    }
...
async function openUiTab(){
  try{
    const url = chrome.runtime.getURL("core/aisthesis.html");
