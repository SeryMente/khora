...
export default function VisorRegistroPage() {
  ...
  // Grouping logic by correlacion_id
  ...
  return (
    ...
    >
      <div className="w-full max-w-5xl space-y-6">
        {/* Header */}
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 gap-4" style={{ borderColor: "var(--khora-border)" }}>
          <div>
        ...
        {/* Control Bar */}
        <div className="flex flex-wrap items-center justify-between gap-4 p-3 border rounded" style={{ borderColor: "var(--khora-border)", background: "var(--khora-surface)" }}>
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer font-bold uppercase">
              <input
                ...
                onChange={(e) => setAgruparPorCorrelacion(e.target.checked)}
                className="rounded"
              />
        ...
        {/* Content list */}
        {loading ? (
          <div className="p-8 text-center text-xs opacity-60">Cargando registro de eventos...</div>
        ) : eventos.length === 0 ? (
          <div className="p-8 text-center text-xs opacity-60 border rounded" style={{ borderColor: "var(--khora-border)" }}>
            ...
            {Object.entries(eventosAgrupados).map(([correlacionId, grupoEvts]) => (
              <section key={correlacionId} className="border rounded p-4 space-y-3" style={{ borderColor: "var(--khora-border)", background: "var(--khora-surface)" }}>
                ...
                <div className="space-y-2">
                  {grupoEvts.map((evt) => (
                    <ItemEvento key={evt.id} evt={evt} expanded={!!expandedDetails[evt.id]} onToggle={() => toggleDetail(evt.id)} />
                  ))}
                </div>
              </section>
            ))}
