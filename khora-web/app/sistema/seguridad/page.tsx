...
function SeguridadContent() {
  ...
  const initialTab = searchParams?.get("tab") === "boveda" ? "boveda" : "seguridad";
  const [activeTab, setActiveTab] = useState<"seguridad" | "boveda">(initialTab);
    ...
    <div className="w-full max-w-3xl space-y-8">
      ...
      >
        <button
          ...
          style={{
            borderBottom: activeTab === "seguridad" ? "2px solid var(--khora-accent)" : "none",
            color: activeTab === "seguridad" ? "var(--khora-ink)" : "var(--khora-accent)",
            opacity: activeTab === "seguridad" ? 1 : 0.6,
          }}
        >
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 font-bold text-sm uppercase transition-colors"
          style={{
            borderBottom: activeTab === "boveda" ? "2px solid var(--khora-accent)" : "none",
            color: activeTab === "boveda" ? "var(--khora-ink)" : "var(--khora-accent)",
            opacity: activeTab === "boveda" ? 1 : 0.6,
          }}
        >
          <LockKeyhole size={18} /> Bóveda
        </button>
      </div>
...
export default function SeguridadPage() {
  return (
    ...
    >
      <Suspense fallback={<div className="text-xs opacity-60">Cargando módulos de seguridad...</div>}>
        <SeguridadContent />
      </Suspense>
    </main>
  );
