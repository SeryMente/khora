...
export function renderAudioStatusBadge(status: string) {
  switch (status) {
    ...
    case "encontrado_no_vinculado":
      return (
        <span className="text-yellow-400 font-semibold text-[10px] flex items-center gap-1">
          🟡 Audio no vinculado
        </span>
      );
    case "incompleto":
    case "audio_parcial":
      return (
        <span className="text-amber-500 font-semibold text-[10px] flex items-center gap-1">
          🟠 Audio incompleto
        </span>
      );
    case "no_aplica":
      return (
        <span className="text-sky-400 font-semibold text-[10px] flex items-center gap-1">
          ⚪ Entrada manual · Audio no esperado
        </span>
      );
    case "no_recuperable":
    default:
      return (
        <span className="text-red-400 font-semibold text-[10px] flex items-center gap-1">
          🔴 Audio no recuperable
        </span>
      );
  }
}
...
export function PipelineView({
  ...
  const filteredItems = pipelineItems.filter((item) => {
    ...
    if (filter === "revision")
      return (
        item.estado === "en_revision" || item.estado === "pendiente_revision"
      );
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--khora-accent)" }}>
            Lectura cómoda, sincronización audio ↔ texto, corrección lingüística
            y compuerta de aprobación autoritativa.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {resumen && (
          <div
            data-ui-id="pipeline.summary"
            className="grid grid-cols-2 md:grid-cols-5 gap-3"
          >
            <div
              className="p-3 border flex flex-col justify-between bg-zinc-950/40"
              style={{ borderColor: "var(--khora-border)" }}
            >
            ...
            <div
              className="p-3 border flex flex-col justify-between col-span-2 md:col-span-1 bg-zinc-950/40"
              style={{ borderColor: "var(--khora-border)" }}
            >
              <span className="text-[10px] font-mono tracking-wider uppercase opacity-60">
                Incidentes Abiertos
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-2xl font-bold text-red-500">
                  {gateDecision?.counts?.incidentes_operativos_abiertos ??
                    resumen.anomalies ??
                    0}
                </span>
              </div>
                  ...
                  style={{
                    backgroundColor:
                      filter === f
                        ...
                        : "var(--khora-surface)",
                    color:
                      filter === f ? "var(--khora-bg)" : "var(--khora-ink)",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[600px]">
          {/* Left Column Index */}
          <div
            data-ui-id="pipeline.list"
            className="lg:col-span-4 flex flex-col space-y-2"
          >
            {loadingPipeline ? (
              <div
                data-ui-id="pipeline.loading-state"
                className="p-8 text-center text-xs opacity-60 border"
                style={{ borderColor: "var(--khora-border)" }}
              >
              ...
              <div
                data-ui-id="pipeline.empty-state"
                className="p-8 text-center text-xs opacity-60 border"
                style={{ borderColor: "var(--khora-border)" }}
              >
                ...
                return (
                  <div
                    ...
                    className={`p-3 border cursor-pointer flex flex-col space-y-1.5 rounded-none transition-all ${
                      isSelected
                        ? "bg-zinc-800/40 ring-1 ring-[var(--khora-accent)]"
                        : "bg-[var(--khora-surface)]"
                    }`}
                    style={{
                  ...
                  >
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <span className="text-[9px] font-mono border px-1 uppercase opacity-80">
                        {item.estado}
                      </span>
                    </div>
          ...
          <div
            data-ui-id="pipeline.drawer"
            className="lg:col-span-8 flex flex-col space-y-4"
          >
            {selectedId && selectedItem ? (
              <div
                className="border p-5 flex flex-col space-y-6 h-full bg-zinc-950/60"
                style={{ borderColor: "var(--khora-border)" }}
              >
                {/* Cockpit Header Bar */}
                <div
                  data-ui-id="revision.cockpit-header"
                    ...
                    <div>
                      ...
                      <h2 className="text-base font-bold font-mono flex items-center gap-2">
                        ...
                        >
                          <Icons.Sparkles
                            size={14}
                            className={generatingTitle ? "animate-spin" : ""}
                          />
                        </button>
                      </h2>
                    </div>
                  ...
                  {gateDecision?.counts && (
                    <div className="grid grid-cols-4 gap-2 text-center text-xs font-mono pt-2">
                      <div className="p-2 border bg-zinc-900/60 border-zinc-800">
                        <div className="opacity-60 text-[9px] uppercase">
                          Tipografía
                        </div>
                        <div className="font-bold text-amber-400">
                          {gateDecision.counts.errores_tipograficos_pendientes}
                        </div>
                      </div>
                      <div className="p-2 border bg-zinc-900/60 border-zinc-800">
                        <div className="opacity-60 text-[9px] uppercase">
                        ...
                        <div className="font-bold text-sky-400">
                          {
                            gateDecision.counts
                              .correcciones_lingüisticas_pendientes
                          }
                        </div>
                      </div>
                      <div className="p-2 border bg-zinc-900/60 border-zinc-800">
                        ...
                        <div className="font-bold text-purple-400">
                          {
                            gateDecision.counts
                              .observaciones_sintacticas_pendientes
                          }
                        </div>
                      </div>
                      <div className="p-2 border bg-zinc-900/60 border-zinc-800">
                ...
                {drawerSubTab === "cockpit" ? (
                  <div className="flex-1 space-y-6">
                    ...
                    {(selectedItem.audio_status === "no_recuperable" ||
                      selectedItem.audio_status === "encontrado_no_vinculado" ||
                      selectedItem.audio_status === "incompleto") && (
                      <div
                        data-ui-id="revision.banner-incidente"
                        className="p-3 border border-red-500/40 bg-red-950/20 text-xs font-mono flex justify-between items-center"
                      >
                        <div className="text-red-300">
                          🔴 Causa de audio detectada:{" "}
                          {selectedItem.audio_status}. Se requiere resolución
                          explícita del operador para habilitar la aprobación.
                        </div>
                        <button
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                      <div className="lg:col-span-8 space-y-3">
                        ...
                        {viewMode === "lectura" ? (
                          <div
                            data-ui-id="revision.reading-prose"
                            className="p-6 border bg-zinc-900/40 border-zinc-800 rounded-none max-w-prose min-h-[280px]"
                          >
                          ...
                          <textarea
                            ...
                            value={editableTexto}
                            onChange={(e) =>
                              actions.onEditableTextoChange?.(e.target.value)
                            }
                            rows={12}
                      ...
                      {/* Side Panel: Findings */}
                      <div
                        data-ui-id="revision.hallazgos-nav"
                        className="lg:col-span-4 space-y-4"
                      >
                        <h3 className="text-xs font-mono uppercase font-bold tracking-wider opacity-70 border-b pb-1">
                          Navegador de Hallazgos (
                          {hallazgos.length > 0
                            ? `${activeHallazgoIndex + 1} de ${hallazgos.length}`
                            : "0 de 0"}
                          )
                        </h3>
                          ...
                          <div className="p-3 border bg-zinc-900 border-zinc-800 space-y-3 font-mono text-xs">
                            <div className="flex justify-between items-center">
                              <span className="font-bold text-amber-300">
                                {activeHallazgo.regla}
                              </span>
                              <span className="uppercase text-[9px] border px-1 border-amber-500/40">
                                {activeHallazgo.tipo_categoria}
                              </span>
                            </div>
                            ...
                            <div className="space-y-1">
                              <div className="text-zinc-400">
                                ...
                                <strong className="text-red-400">
                                  "{activeHallazgo.texto_original}"
                                </strong>
                              </div>
                              <div className="text-zinc-300">
                                Sugerencia:{" "}
                                <strong className="text-emerald-400">
                                  "{activeHallazgo.sugerencia}"
                                </strong>
                              </div>
                              {activeHallazgo.explicacion && (
                            ...
                            <div className="flex justify-between items-center pt-2 border-t border-zinc-800">
                              <div className="flex gap-2">
                                <button
                                  ...
                                  onClick={() =>
                                    actions.onResolveHallazgo?.("rechazar")
                                  }
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    actions.onResolveHallazgo?.("aceptar")
                                  }
                        ...
                        {gateDecision && gateDecision.blockers.length > 0 && (
                          <div
                            data-ui-id="approval.blockers-list"
                            className="space-y-2"
                          >
                            <span className="text-[10px] font-mono text-red-400 uppercase font-bold">
                              Bloqueadores de Aprobación:
                            </span>
                            {gateDecision.blockers.map(
                              (b: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="p-2 border border-red-800/60 bg-red-950/30 text-xs font-mono text-red-300"
                                >
                                  ❌ {b.message}
                                </div>
                              ),
                    ...
                    {/* Audio Player Bar */}
                    <div
                      data-ui-id="revision.audio-player"
                      className="p-3 border bg-zinc-900 border-zinc-800 space-y-2"
                    >
                      <div className="flex justify-between items-center text-xs font-mono">
                        <span className="font-bold flex items-center gap-1">
                          <Icons.Volume2 size={14} /> Reproductor Continuo
                          (Parte {currentPartIndex} /{" "}
                      ...
                      {selectedItem.audio_status !== "no_recuperable" &&
                      selectedItem.audio_status !== "no_aplica" ? (
                        <div className="space-y-3">
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              className="p-2 border border-amber-500/50 bg-amber-950/30 hover:bg-amber-900/50 text-amber-400 font-bold text-xs flex items-center gap-1 cursor-pointer shrink-0 disabled:opacity-40 disabled:cursor-not-allowed"
                            >
                              {isPlaying ? (
                                <Icons.Pause size={14} />
                              ) : (
                            ...
                            <input
                              ...
                              value={currentTimeMs}
                              onChange={(e) =>
                                actions.onGlobalSeek?.(Number(e.target.value))
                              }
                          ...
                          <div className="flex justify-between items-center text-xs font-mono pt-1 border-t border-zinc-800">
                            ...
                            <span className="opacity-80">
                              Parte {currentPartIndex} de{" "}
                              {manifiestoPartes.length || 1}
                            </span>
                            <button
                              disabled={
                                currentPartIndex >= manifiestoPartes.length
                              }
                              onClick={() =>
                                actions.onManualPartChange?.(
                                  currentPartIndex + 1,
                        ...
                        <div className="text-xs font-mono opacity-60 italic">
                          Audio no disponible para reproducción.
                        </div>
                      )}
                    </div>
                    ...
                    <div
                      data-ui-id="approval.gate"
                      className="p-4 border bg-zinc-900/60 border-zinc-800 space-y-4 font-mono"
                    >
                      <div className="flex justify-between items-center border-b pb-2 border-zinc-800">
                        <div>
                          <h4 className="font-bold text-xs uppercase tracking-wider">
                            Compuerta de Aprobación Server-Side
                          </h4>
                          <span className="text-[10px] opacity-60">
                            hash: {gateDecision?.gate_hash || "evaluando..."}
                          </span>
                        </div>
                        ...
                        selectedItem.estado === "ingerido" ? (
                          <span
                            data-ui-id="approval.badge-approved"
                            className="text-xs font-bold text-emerald-400 border border-emerald-500/40 bg-emerald-950/40 px-2 py-1"
                          >
                            ✓ APROBADO v{selectedItem.version_aprobada || 1}
                          </span>
                        ) : (
                          <span
                            className={`text-xs font-bold px-2 py-1 border ${gateDecision?.canApprove ? "text-emerald-400 border-emerald-500/40 bg-emerald-950/40" : "text-red-400 border-red-500/40 bg-red-950/40"}`}
                          >
                            {gateDecision?.canApprove
                              ? "Habilitado para Aprobación"
                              : "Bloqueado"}
                          </span>
                        )}
                      </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex flex-col sm:flex-row gap-3 items-center">
                              <button
                                ...
                                disabled={
                                  !gateDecision?.canApprove || approvingVersion
                                }
                              ...
                              <button
                                ...
                                onClick={() =>
                                  actions.onSetShowAccessibleModal?.(true)
                                }
                                disabled={
                                  !gateDecision?.canApprove || approvingVersion
                                }
                      ...
                      {/* Ingestion Section */}
                      <div
                        data-ui-id="ingesta.container"
                        className="pt-2 border-t border-zinc-800 space-y-2"
                      >
                        {(selectedItem.estado === "listo_ingesta" ||
                        ...
                        {ingesting && (
                          <div
                            data-ui-id="ingesta.status-running"
                            className="p-2 text-xs font-mono text-amber-300"
                          >
                            Ejecutando propuesta de ingesta en el grafo de
                            conocimiento...
                          </div>
                        )}
                        ...
                        {ingestaResult && ingestaResult.success && (
                          <div
                            data-ui-id="ingesta.result-success"
                            className="p-2.5 text-xs font-mono border border-emerald-600 bg-emerald-950/60 text-emerald-300"
                          >
                        ...
                        {ingestaResult && !ingestaResult.success && (
                          <div
                            data-ui-id="ingesta.result-failure"
                            className="p-2.5 text-xs font-mono border border-red-600 bg-red-950/60 text-red-300"
                          >
                  ...
                  <div className="space-y-4 font-mono text-xs">
                    <div className="p-3 border bg-zinc-950/40 border-zinc-800 space-y-2">
                      ...
                      <div>
                        Versión Aprobada:{" "}
                        {selectedItem.version_aprobada
                          ? `v${selectedItem.version_aprobada}`
                          : "Ninguna"}
                      </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="border p-6 max-w-md w-full bg-zinc-950 border-zinc-700 space-y-4 font-mono text-xs">
            <h3 className="text-sm font-bold uppercase text-red-400">
              Resolución de Incidente de Audio
            </h3>
            <p className="text-zinc-300">
              Selecciona la causa explícita para resolver la ausencia de audio
              en este volcado:
            </p>
            <select
              value={selectedAudioResolveCode}
              onChange={(e) =>
                actions.onSetSelectedAudioResolveCode?.(e.target.value)
              }
              className="w-full p-2 border bg-zinc-900 border-zinc-700 text-zinc-200"
            >
              <option value="aceptado_sin_audio">
                aceptado_sin_audio (Aceptar conscientemente sin audio)
              </option>
              <option value="audio_recuperado">
                audio_recuperado (Audio recuperado)
              </option>
            </select>
            <div className="flex justify-end gap-2 pt-2">
              <button
                onClick={() => actions.onSetShowAudioResolveModal?.(false)}
                className="px-3 py-1.5 border border-zinc-700"
              >
                Cancelar
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="border p-6 max-w-md w-full bg-zinc-950 border-zinc-700 space-y-4 font-mono text-xs">
            <h3 className="text-sm font-bold uppercase text-amber-400">
              Confirmación Accesible de Aprobación
            </h3>
            <p className="text-zinc-300">
              Escribe exactamente{" "}
              <strong className="text-emerald-400">APROBAR v1</strong> para
            ...
            <input
              ...
              onChange={(e) =>
                actions.onSetAccessibleConfirmText?.(e.target.value)
              }
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                onClick={() => actions.onSetShowAccessibleModal?.(false)}
                className="px-3 py-1.5 border border-zinc-700"
              >
                Cancelar
              </button>
              <button
                disabled={
                  accessibleConfirmText !== "APROBAR v1" || approvingVersion
                }
