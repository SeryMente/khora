...
import { useLocalCapturas } from "@/lib/hooks";
...
export default function Capturar() {
  ...
  const [serverCapturas, setServerCapturas] = useState<ServerCaptura[]>([]);
  ...
  const { capturas: localCapturas, loading: loadingLocal, addCaptura } = useLocalCapturas();
  ...
  useEffect(() => {
    cargarCapturasServidor();
  }, [guardado]);

  const cargarCapturasServidor = async () => {
  ...
  const getStatusMessage = (status: string) => {
    switch (status) {
      case "synced":
        return { text: "sincronizado", color: "text-green-500" };
      case "pending":
        return { text: "pendiente", color: "text-yellow-500" };
      case "offline":
        return { text: "sin conexión — se sincronizará", color: "text-orange-500" };
      default:
        return { text: "", color: "" };
    }
  };

  // Combinar capturas: locales primero, luego del servidor (evitando duplicados)
  const localIds = new Set(localCapturas.map((c) => c.id));
  const allCapturas = [
    ...localCapturas,
    ...serverCapturas.filter((c) => !localIds.has(c.id)),
  ];
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="py-8 px-4">
        <div className="max-w-md mx-auto">
          {/* Loading */}
          {(cargandoTimeline || loadingLocal) && allCapturas.length === 0 && (
          ...
          {/* Error */}
          {errorTimeline && allCapturas.length === 0 && (
            <div className="text-center">
              <p className="text-gray-400 text-sm mb-3">{errorTimeline}</p>
              <button
                onClick={cargarCapturasServidor}
          ...
          {/* Empty state */}
          {!cargandoTimeline && !loadingLocal && allCapturas.length === 0 && !errorTimeline && (
          ...
          {/* Timeline */}
          {allCapturas.length > 0 && (
            <ul className="space-y-3" role="list">
              {allCapturas.map((captura) => {
                const status =
                  "status" in captura ? (captura.status as string) : "synced";
